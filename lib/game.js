...
function emptyRoomState() {
    return { mission: "", drawnAt: 0, drawnBy: "", owner: "", ownerAt: 0, votes: {}, players: {}, turnOrder: [], resetAt: 0 };
}
...
function clearLocalBingoStorage() {
    try {
        sessionStorage.removeItem("onepiece_bingo_player_name");
        const toRemove = [];
        for (let i = 0; i < localStorage.length; i += 1) {
            const key = localStorage.key(i);
            if (key && key.startsWith("onepiece_bingo_")) toRemove.push(key);
        }
...
function handleClearAllData() {
    ...
    const confirmed = window.confirm(
        "ล้างข้อมูลทั้งหมดในห้องนี้?\nClear all room data: mission, votes, crew, board marks, and your captain name on this device?"
    );
    if (!confirmed) return;
    ...
    if (captainNameSpan) captainNameSpan.innerText = "Guest";
...
function renderMissionLine(missionText) {
    const prefixEl = document.getElementById("missionOwnerPrefix");
    ...
    const hasMission = !!(currentQuestionText && currentQuestionText.trim()) && text !== idleMissionText();
    const owner = ((hasMission ? (sharedDrawnBy || sharedOwner) : sharedOwner) || "").trim();

    if (prefixEl) prefixEl.textContent = owner ? owner + " : " : "";
    if (textEl) textEl.textContent = text;
    else if (drawName) drawName.textContent = owner ? owner + " : " + text : text;
}
...
function updateOwnerMissionUI() {
    ...
    if (drawHint) {
        if (!isNamedCaptain(currentCaptainName())) {
            drawHint.textContent = "กรอกชื่อกัปตันก่อน";
        } else if (allowed && nextName) {
            drawHint.textContent = "ตาคุณสุ่มภารกิจ · ส่งต่อ " + nextName;
        } else if (allowed) {
            drawHint.textContent = "กด Draw Mission เพื่อสุ่มภารกิจ";
        } else {
            drawHint.textContent = "รอภารกิจจาก " + ownerDisplayName();
        }
    }
        ...
        mqttClient.on("connect", () => {
            ...
            setTimeout(() => {
                if (!receivedMqtt) {
                    ...
                    if (cached.mission || cached.owner || cached.resetAt || Object.keys(cached.players || {}).length) {
                        publishMqtt(cached);
                    }
                }
            }, 900);
...
function updateVoteUI() {
    ...
    setVoteRosterVisible(yesNames.length > 0 || noNames.length > 0);
}

function setVoteRosterVisible(show) {
    const roster = document.getElementById("voteRoster");
    if (roster) roster.hidden = !show;
}
